...
function parseItemCard(el: HTMLElement): BoothItem | null {
  ...
  const shopEl = queryFirst(el, [
    ".item-card__shop-name",
    ".shop-name",
    "[data-shop-name]",
  ]);
  const shopName =
    shopEl?.getAttribute("data-shop-name") ||
    shopEl?.textContent?.trim() ||
    null;
  ...
  const categoryName =
    catEl?.getAttribute("data-category") ||
...
export function parseItemDetailHtml(
  html: string,
  itemId: number,
): BoothItem | null {
  const doc = new DOMParser().parseFromString(html, "text/html");
  ...
  const tags: string[] = [];
  doc
    .querySelectorAll(".item-tag a, a.tag, .item-info-tag a, .tag-list a")
    .forEach((el) => {
      const t = el.textContent?.trim();
      if (t && !tags.includes(t)) tags.push(t);
    });

  const catEl = queryFirstDoc(doc, [
    ".item-category a",
    ".category-name a",
    "[data-category]",
  ]);
